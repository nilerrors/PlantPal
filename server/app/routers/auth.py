...
async def signup(background_tasks: BackgroundTasks, user: schemas.auth.UserSignup):
...
async def resend_verification(background_tasks: BackgroundTasks, user: schemas.auth.UserResendVerification):
    db_user = await crud.auth.get_user_by_email(user.email)
...
async def get_user_id(user: schemas.auth.UserLogin):
    user_id = await crud.auth.get_user_by_email_password(user.email, user.password)
    if user_id is None:
...
async def update_user(user: schemas.auth.UserUpdate, Authorize: AuthJWT = Depends()):
    Authorize.jwt_required()
...
async def update_user_password(user: schemas.auth.UserUpdatePassword, Authorize: AuthJWT = Depends()):
    Authorize.jwt_required()
...
@router.delete('/user')
async def delete_user(user: schemas.auth.UserRemove, Authorize: AuthJWT = Depends()):
    Authorize.jwt_required()
