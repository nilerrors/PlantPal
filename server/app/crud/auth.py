...
async def get_user(user_id: str):
    return await prisma.user.find_first(where={'id': user_id},
...
async def authenticate_user(user: schemas.auth.UserLogin):
    ...
    if db_user is None or not hasher.verify_password(user.password,
                                                     db_user.password):
        return False
    return True
...
async def remove_user(user: schemas.auth.UserRemove):
    ...
    deleted_user = await prisma.user.delete(where={'id': delete_user.id})
